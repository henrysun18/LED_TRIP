/*
Names: Henry Sun & Michael Zhang
Mrs. Strekovska; TEJ4M1
Due Date: Tuesday, January 21, 2014
Computer Engineering 12 Final Project: "LED_TRIP"
*******************************************
*    _    ____ ___      ___ ____ _ ___    *
*    |    |___ |  \      |  |__/ | |__]   *
*    |___ |___ |__/ ___  |  |  \ | |      *
*                                         *
*******************************************                                   
...
void loop(){
  resetVariables(); //note that LDR is calibrated even before start screen is shown
  startScreen();
  displayText(Ready);
  game();
  delay(2000);//wait a while before resetting the game after loss
  //Resets both active & inactive screens of the LoL Shield
  LedSign::Flip(1);
  LedSign::Clear();
  LedSign::Flip(1);
  LedSign::Clear();
...
void startScreen(){
  ...
  do{ //cannot skip the start screen by holding button
    LedSign::Flip(1);
    delay(800);
...
void game(){
  while (1==1){
    ...
    if (ax==-3||bx==-4||cx==-7){ //recycles Obstacle motion
      ...
      if (falling) ay=TrueRandom.random(6); //in tilt mode, spawn the 4x4 block in a random spot
      score++;
    }
    // Take PLAYER input
    ...
    if (analogRead(PRESSURE)>2){ //pretty much the higher the jump, the higher the priority
      maxHeight=5;
    ...
    if (falling){
      currentObstacle=0; //only spawns 4x4 blocksin tilt mode
      detectTilt(); //override all controls w/ tilt IF game switches to tilt game (since dpy is not used until later)
    }
    if (!warn&&score%5!=0) warn=true; //no infinite looping the text!
    if (warn&&score%5==0){
      if (score%10==0){ //Run[] given priority when score=0
        py=6;
        ay=5;
        falling=false;
        displayText(Run); //every 10th level, Run game for 5 levels
        frameDelay-=8; //5 levels = difficulty increase!
      }
      else if (score%5==0){
        py=3; 
        falling=true;
        displayText(Tilt); //the other half of the levels will be spent playing Tilt game
        frameDelay-=8;
      }
      warn=false; //only shows warning every 5 levels
    }
    
    // Move the PLAYER & OBSTACLES (IF THEY ARE IN PLAY)
    py=py+dpy; 
    ...
    for (int8_t y=py; y<py+3; y++)
      for (int8_t x=px; x<px+2; x++)
        if (y>=0&&y<9) LedSign::Set(x,y,3); //(restrict y because the LDR makes the player go out of bounds)
        
    // Draw Obstacle as well as Player  
    drawObstacle();
  
    // Next frame (shows what was just drawn)
    LedSign::Flip();
    
    // Detect collision for the obstacle onscreen
    detectCollision();
    delay(frameDelay); //the time between frames, shorter=harder
    ...
    if (lives==0){
      ...
      displayText(Score);
      break; //exit the while(1==1) in game() and go back to loop()
    }
  }
}
...
void detectTilt() {
  ...
  if (tilt1<00&&tilt2>00) //tilting left
    if (py+2==8) dpy=0; else dpy=1; //prevents going too far left 
  else if (tilt1>00&&tilt2<00) 
    if (py==0) dpy=0; else dpy=-1; //prevents going too far right
  else
