...
int8_t score=0;
int8_t lives=2; //start with as many lives as you want, we chose 5
...
void setup(){
  ...
  cy=0;
  dcx=-1;
  threshold=analogRead(LDR)-200; //auto-calibrating LDR
  Serial.begin(9600);
}
...
void game(){
  while (1){
    ...
    detectCollision();
    delay(frameDelay);
    Serial.print(char(score));
    if (lives==0){
      delay(2000);
      Score[0]=itoa(score/100);
      Score[1]=itoa(score/10);
      Score[2]=itoa(score%10);
      displayText(YouLose);
...
void displayText(char Message[9]){
  LedSign::Clear();
  int8_t limit=0;
