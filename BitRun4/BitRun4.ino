...
static const char Tilt[]="TILT      ";
const int8_t BUTTON = A0;
const int8_t PRESSURE = A1;
const int8_t LDR = A2;
const int8_t TILT1 = A4;
const int8_t TILT2 = A5;
int8_t frameDelay=100; //time between each frame, basically a difficulty bar
int8_t tilt1,tilt2;
int8_t px,py,dpy,maxHeight; //Player X/Y coordinate & speed
int8_t score=0;
int8_t ax,ay,dax; //obstacles
int8_t bx,by,dbx;
int8_t cx,cy,dcx,threshold;
boolean falling=false; //false=running game; true=tilt-sensing game
boolean warn=false;
int8_t currentObstacle=0; //Provides the sequence of random obstacles
...
void setup(){
  ...
  cy=0;
  dcx=-1;
  threshold=100; //set threshold here
  //(for some reason, LDR reading is significantly less during start-up
  //so the initial reading in itself will be used as the threshold)
  Serial.begin(9600);
}
...
void game(){
  while (1){
    ...
    if (warn&&score%5==0&&score>9){
      if (score%10==0){
        py=3; 
        falling=true;
        tiltWarning(); //after 10 levels, begin tilt game
        frameDelay-=8;
      }
      else if (score%5==0){
        py=6;
        ay=5;
        falling=false;
        runWarning(); //after 5 more levels, back to running
        frameDelay-=8;
      }
...
void runWarning(){
  ...
  int8_t limit=0;
	...
	int8_t w = Font::Draw(Run[i2], x2, 0);
        x2 += w, i2 = (i2+1)%strlen(Run);
	if (x2 <= 0)	
	x = x2, i = i2;
      }
      LedSign::Flip();
...
void tiltWarning(){
  ...
  int8_t limit=0;
    for (int8_t x=DISPLAY_COLS, i=0;; x--) {
      ...
      LedSign::Clear();
        for (int8_t x2=x, i2=i; x2<DISPLAY_COLS;) {
	  int8_t w = Font::Draw(Tilt[i2], x2, 0);
	  x2 += w, i2 = (i2+1)%strlen(Tilt);
	  if (x2 <= 0)	
  	    x = x2, i = i2;
	}
      LedSign::Flip();
